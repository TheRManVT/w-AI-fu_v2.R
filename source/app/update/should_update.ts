...
export async function checkUpdates(): Promise<void> {
    ...
    try {
        ...
        IO.warn(
            "Error: Could not contact github while trying to get latest version."
        );
        return;
    }
    ...
    if (typeof data !== "object" || data === null || data === undefined) {
        IO.warn(
            "Error: Fetched invalid data from github while trying to retreive latest version."
        );
        return;
    }
    ...
    if (
        "name" in latest_version &&
        wAIfu.getVersion() !== latest_version["name"]
    ) {
